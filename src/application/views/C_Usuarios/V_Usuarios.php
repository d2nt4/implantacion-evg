				...
				<div class="col-9" >
					...
					<?php
						...
						foreach($this->listaUsuarios as $indice => $valor)
							echo '<tr><td><p id="valor"> '.$valor.'</p></td><td><a href="'.base_url().'C_GestionEVG/modificarUsuarioForm/'.$indice.'" tabindex=\'-1\'><button>Modificar</button></a><button onclick="confirmar(\'¿Seguro que quieres borrar el usuario: <b>'.$valor.'</b>?\',\''.base_url().'C_GestionEVG/borrarUsuario/'.$indice.'\', \'Eliminar Usuario\', \'Cancelar\', \'Eliminar\')" data-toggle="modal" data-target="#myModal">Borrar</button></td></tr>';
						echo '</table>'
					?>
				</div>
			</div>
